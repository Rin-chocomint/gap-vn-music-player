...
Letak tombol **Releases** ada disebelah kanan halaman ini, jika tidak kelihatan scroll saja ke atas. Pastikan kamu tidak mengambil versi lama (v0.0.0.6) untuk tau kemajuan sebenarnya aplikasi ini.
...
3.  **Jelajahi Kode:** Logika utama engine terdapat di `main.js` (Cari fungsi `Rin.js` atau logika player).
4.  **Custom:** Template tampilan visual novel ada di `vn_player_template.html`.

> **Catatan:** Folder `aset` di repositori ini hanya berisi file *dummy* (sampel). Silakan ganti dengan aset kreatif Anda sendiri saat membuat proyek baru.
